function list_detalle_registro_venta(){
    $.ajax({
        url:"controller_func/detalle_registro_venta/list.php"
    }).done(function(data){
...
$(document).on('click', '.nuevo_detalle_venta_modal',function(){
    var id=$(this).data("id");
    var url="controller_func/detalle_registro_venta/create.php?id="+id;
    $.get(url, function(data){
        $("#form_detalle_venta").empty();
        $("#form_detalle_venta").append(data);
        if(id>0){
            $(".title_marca").empty();
            $(".title_marca").append("Modificar Detalle Registro Venta");
        }else{
            $(".title_marca").empty();
            $(".title_marca").append("Nuevo Detalle Registro Venta");
        }
...
$(document).on('click', '.activar-item', function() {
    ...
    $.ajax({
        type:'POST',
        url:'controller_func/detalle_registro_venta/accion.php',
        data:data,
     ...
     $.ajax({
        ...
        success:function(data){     
            if(data.trim()=="true_create"){               
                ...
                toastr.success("Se creó Detalle venta");
                list_detalle_registro_venta();
            }else if(data.trim()=="true_update"){
                $("#form_detalle_venta").empty();
                $("#modal-form-detalle-venta").modal("hide");
                toastr.success("Se actualizó el Detalle venta");
                list_detalle_registro_venta();                
...
$(document).on('click', '.show_detalle_venta_modal', function() {
    var id=$(this).data("id");
    var url="controller_func/detalle_registro_venta/show.php?id="+id;
    $.get(url, function(data){
