...
$(document).on('click', '#btn_save', function() {
    ...
    var strGenero = document.querySelector('#txt_genero').value;
    ...
    $.ajax({
        ...
        success:function(data){
            if(data.trim()=="true_create"){               
                $("#form_genero").empty();
                $("#modal-form-genero").modal("hide");
                toastr.success("Se agrego un genero");
                list_genero();
                
            }else if(data.trim()=="true_update"){
                $("#form_genero").empty();
                $("#modal-form-genero").modal("hide");
                toastr.success("Se actualizo genero");
                list_genero();                
...
/* ------Modal SHOW vista------*/
function list_genero()
{
    $.ajax({
        url:"controller_func/genero/list.php"
    }).done(function(data){
        $('.table-genero').empty();
        $('.table-genero').append(data);        
    })
} 

/* ------Modal formulario Crear y Editar------*/
$(document).on('click', '.new-modal-genero', function() {
    var id=$(this).data("id");
    ...
    $.get(url, function(data){
        ...
        if(id>0){
            $(".modal-title").empty();
            $(".modal-title").append("Modificar genero");
        }else{
            $(".modal-title").empty();
            $(".modal-title").append("Nuevo genero");
        }
        $("#modal-form-genero").modal("show");
    })
});

/* ------Modal SHOW vista------*/
$(document).on('click', '.new-modal-show-genero', function() {
    var id=$(this).data("id");
    var url="controller_func/genero/show.php?id="+id;
    $.get(url, function(data){
        $("#form_show_genero").empty();
        $("#form_show_genero").append(data);
        $("#form_show_genero :input").prop('disabled',true);
        $(".modal-title").empty();
        $(".modal-title").append("Detalles genero");
        $("#modal-form-show-genero").modal("show");
    })
});
