...
function editararbolvirtual(){
    ...
    $.get({url:"controller_func/solicitud_arbol_virtual/consultar_edit.php",data: dataString,type: 'POST'}).done(function (response) {
        ...
        chart.on('drop', function (sender, draggedNodeId, droppedNodeId) {
            ...
            if(sender.nodes[droppedNodeId]["children"].length>=0 && sender.nodes[droppedNodeId]["children"].length<=5){
                array.push(movingNode);
            }else{
                toastr.error('Representante ya completo sus '+sender.nodes[droppedNodeId]["children"].length+' afiliados directos, proceda a eliminar o mover sus directos');
                return false;
            }
        });
        ...
        chart.on('redraw', function(sender){
            ...
            for(var i = 0; i < nodeElements.length; i++){
                ...
                nodeElement.ondrop = function(ev) {    
                    ...
                    if(cantafi>=0 && cantafi<=5){
                         
                    }else{
                        toastr.error('Representante ya tiene completo sus '+cantafi+' afiliados directos, proceda a eliminar o mover sus directos');
                        return false;
                    }                    
