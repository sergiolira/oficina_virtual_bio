...
function list_agente_bmi(){
    $.ajax({
        url:"controller_func/agente_bmi/list.php"
    }).done(function(data){
        $('.table-agente').empty();
        $('.table-agente').append(data);        
    })
}

/* ------Modal formulario Crear y Editar------*/
$(document).on('click', '.new-modal-agente', function() {
    document.getElementById('btn_save').disabled = true;
    var id=$(this).data("id");
    var url="controller_func/agente_bmi/create.php?id="+id;
    $.get(url, function(data){
        $("#form_agente").empty();
        $("#form_agente").append(data);
        if(id>0){
            $(".modal-title").empty();
            $(".modal-title").append("Modificar Agente");
            document.getElementById('btn_save').disabled = false;
        }else{
            $(".modal-title").empty();
            $(".modal-title").append("Nuevo Agente");
        }
        $("#modal-form-agente").modal("show");
    })
});

$(document).on('click', '#btn_save', function() {
    var data=$('#form_agente').serialize();
     $.ajax({
        type:'POST',
        url:'controller_func/agente_bmi/accion.php',
        data:data, 
        success:function(data){
            if(data.trim()=="true_create"){               
                $("#form_agente").empty();
                $("#modal-form-agente").modal("hide");
                toastr.success("Se creó un nuevo Agente");
                list_agente_bmi();
                
            }else if(data.trim()=="true_update"){
                $("#form_agente").empty();
                $("#modal-form-agente").modal("hide");
                toastr.success("Se actualizo el Agente");
                list_agente_bmi();                
...
/* ------Modal SHOW vista------*/
$(document).on('click', '.new-modal-show-agente', function() {
    var id=$(this).data("id");
    var url="controller_func/agente_bmi/show.php?id="+id;
    $.get(url, function(data){
        $("#form_show_agente").empty();
        $("#form_show_agente").append(data);
        $("#form_show_agente :input").prop('disabled',true);
        $(".modal-title").empty();
        $(".modal-title").append("Detalles del Agente");
        $("#modal-form-show-agente").modal("show");
    })
});
...
//export data excel
$(document).on('click', '.exportar-excel-agente-bmi', function() {
    $.ajax({
        type:'POST',
        url:'controller_func/agente_bmi/exportar_excel.php',
        dataType:'json',
        beforeSend:function(){
            cargar_data();
        },complete:function(){
