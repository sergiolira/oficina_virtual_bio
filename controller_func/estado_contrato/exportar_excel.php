...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento estado de contrato Office 2007 XLSX")
							 ->setSubject("Documento estado de contrato Office 2007 XLSX")
							 ->setDescription("Documento estado de contrato para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento estado de contrato");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Estado de contrato')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_estado_contrato"])
	            ->setCellValue("C$i", $fila["estado_contrato"])
...
$response =  array(
        ...
        'namearchivo'=>'Estado_contrato_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
