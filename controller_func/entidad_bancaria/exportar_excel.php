...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento entidad bancaria Office 2007 XLSX")
							 ->setSubject("Documento entidad bancaria Office 2007 XLSX")
							 ->setDescription("Documento entidad bancaria para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento entidad bancaria");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Entidad bancaria')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_entidad_bancaria"])
	            ->setCellValue("C$i", $fila["entidad_bancaria"])
...
$response =  array(
        ...
        'namearchivo'=>'Entidad_bancaria_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
