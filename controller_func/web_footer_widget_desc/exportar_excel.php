...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento web footer widget desc Office 2007 XLSX")
							 ->setSubject("Documento web footer widget desc Office 2007 XLSX")
							 ->setDescription("Documento web footer widget desc para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento web footer widget desc");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Web footer widget desc')
			->setCellValue('D1', 'Web footer widget')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_web_footer_widget_desc"])
	            ->setCellValue("C$i", $fila["web_footer_widget_desc"])
				->setCellValue("D$i", $fila["web_footer_widget"])
...
$response =  array(
        ...
        'namearchivo'=>'Web_footer_widget_desc_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
