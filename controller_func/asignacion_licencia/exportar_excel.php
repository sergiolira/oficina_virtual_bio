...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento asignación de licencia Office 2007 XLSX")
							 ->setSubject("Documento asignación de licencia Office 2007 XLSX")
							 ->setDescription("Documento asignación de licencia para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento asignación de licencia");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Herramienta Tecnologica')
            ->setCellValue('D1', 'Licencia')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_asignacion_licencia"])
	            ->setCellValue("C$i", $fila["descripcion"])
				->setCellValue("D$i", $fila["licencia"])
...
$response =  array(
        ...
        'namearchivo'=>'Asignación_licencia_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
