...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento Web del menu Office 2007 XLSX")
							 ->setSubject("Documento Web del menu Office 2007 XLSX")
							 ->setDescription("Documento Web del menu para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento Web del menu");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Web del menu')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_web_menu"])
	            ->setCellValue("C$i", $fila["web_menu"])
...
$response =  array(
        ...
        'namearchivo'=>'Web_menu_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
