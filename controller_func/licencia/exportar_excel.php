...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento licencia Office 2007 XLSX")
							 ->setSubject("Documento licencia Office 2007 XLSX")
							 ->setDescription("Documento licencia para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento licencia");
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D1', 'Tipo de Licencia')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_licencia"])
	            ->setCellValue("C$i", $fila["licencia"])
				->setCellValue("D$i", $fila["tipo_licencia"])
				->setCellValue("E$i", $fila["codigo"])
				->setCellValue("F$i", $fila["stock"])
	            ->setCellValue("G$i", $estado)
	            ->setCellValue("H$i", $fila["fecharegistro"])
	            ->setCellValue("I$i", $fila["id_usuarioregistro"]);
	            $i++;
}
...
$response =  array(
        ...
        'namearchivo'=>'Licencia_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
