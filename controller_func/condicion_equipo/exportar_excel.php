...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento condicion de equipo Office 2007 XLSX")
							 ->setSubject("Documento condicion de equipo Office 2007 XLSX")
							 ->setDescription("Documento condicion de equipo para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento condicion de equipo");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Condición de Equipo')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_condicion_equipo"])
	            ->setCellValue("C$i", $fila["condicion_equipo"])
...
$response =  array(
        ...
        'namearchivo'=>'Condicion_equipo_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
