...
?>
<table id="table_det_comisiones_list" class="table table-bordered table-striped table-sm" style="font-size: 20px;">
    <thead>
        <tr>
            <th>N°</th>
            <th>ID</th>
    ...
    <tbody>
        <?php while ($fila = mysqli_fetch_assoc($rs)) {
            ...
            if($fila["estado"]==1){
                ...
                $btn_eliminar='<button data-id="'.$fila["id_cabacera_comisiones_venta"].'"   
                title="Inactivar" class="btn btn-xs btn-danger desactivar-item" id="btn_c_c'.$fila["id_cabacera_comisiones_venta"].'">
                <i class="far fa-trash-alt" id="icon_c_c'.$fila["id_cabacera_comisiones_venta"].'"></i></button>';
              }
              else{ 
                $estado="Inactivo";$estado_style="class='text-danger'";
                $btn_eliminar="";
                $btn_eliminar='<button data-id="'.$fila["id_cabacera_comisiones_venta"].'"   
                title="Activar" class="btn btn-xs btn-success activar-item" id="btn_c_c'.$fila["id_cabacera_comisiones_venta"].'">
                <i class="fas fa-user-check" id="icon_c_c'.$fila["id_cabacera_comisiones_venta"].'"></i>
              </button>';
              }
              if($_SESSION['actualizar']==1){
                $btn_editar='<button data-id="'.$fila["id_cabacera_comisiones_venta"].'"
...
';
            ...
            <tr>
                ...
                <td>
                    ...
                    <button data-id="<?php echo $fila['id_cabacera_comisiones_venta'] ?>" title="Ver" class="btn btn-xs btn-primary new-modal-show-cabecera"><i class="far fa-eye"></i></button>
