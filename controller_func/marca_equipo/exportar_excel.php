...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento marca de equipo Office 2007 XLSX")
							 ->setSubject("Documento marca de equipo Office 2007 XLSX")
							 ->setDescription("Documento marca de equipo para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento marca de equipo");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Marca de equipo')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_marca_equipo"])
	            ->setCellValue("C$i", $fila["marca_equipo"])
...
$response =  array(
        ...
        'namearchivo'=>'Marca_equipo_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
