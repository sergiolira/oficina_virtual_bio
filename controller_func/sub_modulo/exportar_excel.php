...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento sub_modulo Office 2007 XLSX")
							 ->setSubject("Documento sub_modulo Office 2007 XLSX")
							 ->setDescription("Documento sub_modulo para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento sub_modulo");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'sub modulo')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_sub_modulo"])
	            ->setCellValue("C$i", $fila["sub_modulo"])
	            ->setCellValue("D$i", $fila["modulo"])
	            ->setCellValue("E$i", $fila["nivel"])
...
$response =  array(
        ...
        'namearchivo'=>'sub_modulo_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
