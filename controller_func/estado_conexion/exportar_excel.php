...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento estado conexion Office 2007 XLSX")
							 ->setSubject("Documento estado conexion Office 2007 XLSX")
							 ->setDescription("Documento estado conexion para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento estado conexion");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Estado conexion')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_estado_conexion"])
	            ->setCellValue("C$i", $fila["estado_conexion"])
...
$response =  array(
        ...
        'namearchivo'=>'Estado_conexion_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
