...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento estado programacion charla Office 2007 XLSX")
							 ->setSubject("Documento estado programacion charla Office 2007 XLSX")
							 ->setDescription("Documento estado programacion charla para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento estado programacion charla");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Estado programacion charla')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_estado_programacion_charla"])
	            ->setCellValue("C$i", $fila["estado_programacion_charla"])
...
$response =  array(
        ...
        'namearchivo'=>'Estado_Programacion_Charla_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
