...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento zona de supervision Office 2007 XLSX")
							 ->setSubject("Documento zona de supervision Office 2007 XLSX")
							 ->setDescription("Documento zona de supervision para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento zona de supervision");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Zona de supervision')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_zona_supervision"])
	            ->setCellValue("C$i", $fila["zona_supervision"])
...
$response =  array(
        ...
        'namearchivo'=>'Zona_supervision_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
