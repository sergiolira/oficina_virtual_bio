...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento tipos de licencia Office 2007 XLSX")
							 ->setSubject("Documento tipos de licencia Office 2007 XLSX")
							 ->setDescription("Documento tipos de licencia para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento tipos de licencia");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Tipo de Licencia')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_tipo_licencia"])
	            ->setCellValue("C$i", $fila["tipo_licencia"])
...
$response =  array(
        ...
        'namearchivo'=>'Tipo_licencia_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
