...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento tipo de equipo Office 2007 XLSX")
							 ->setSubject("Documento tipo de equipo Office 2007 XLSX")
							 ->setDescription("Documento tipo de equipo para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento tipo de equipo");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'Tipo de Equipo')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_tipo_equipo"])
	            ->setCellValue("C$i", $fila["tipo_equipo"])
...
$response =  array(
        ...
        'namearchivo'=>'Tipo_equipo_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
