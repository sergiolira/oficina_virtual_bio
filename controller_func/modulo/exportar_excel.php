...
$objPHPExcel->getProperties()->setCreator("Freky")
							 ...
							 ->setTitle("Documento modulo Office 2007 XLSX")
							 ->setSubject("Documento modulo Office 2007 XLSX")
							 ->setDescription("Documento modulo para Office 2007 XLSX, generado usando clases PHP.")
							 ->setKeywords("Office 2007 openxml php")
							 ->setCategory("Documento modulo");

// Agrega algunos datos (Titulos)
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Nº')
            ->setCellValue('B1', 'ID')
            ->setCellValue('C1', 'modulo')
...
while($fila=mysqli_fetch_assoc($rs)){
	...
	$objPHPExcel->setActiveSheetIndex(0)
	            ->setCellValue("A$i", $c)
	            ->setCellValue("B$i", $fila["id_modulo"])
	            ->setCellValue("C$i", $fila["modulo"])
...
$response =  array(
        ...
        'namearchivo'=>'modulo_'.date('dmy')."_".date('Hs')."_".rand(10, 99)
    );
